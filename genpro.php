...
function genpro_activation_function() {
    ...
    $sql = "CREATE TABLE $table_name (
     ...
     serial mediumint(255),
     guarantee mediumint(255),
     reg_date date,
     UNIQUE KEY id (id)
    ) $charset_collate;";
...
function genpro_products_list_display(){
    ...
    $delete = 0;
    global $wpdb;
	$final_product_query =  "select * from wp_genpro_products";
    ...
    if ( ! isset( $_POST['nonce_submit'] ) || ! wp_verify_nonce( $_POST['nonce_submit'], 'nonce_submit_action' ) ) {
        ...
        if($_SERVER["REQUEST_METHOD"] == "POST") {
           if(isset($_POST['genpro_delete'])){
                ...
                $delete_product_query =  "DELETE from wp_genpro_products where serial = '$genpro_serial_delete'";
             ...
             $dt2 = new DateTime();
                ...
                if(!$exist){
                    $insert_product_query =  "INSERT INTO wp_genpro_products (serial, guarantee, reg_date) VALUES ('$genpro_serial', '$genpro_guarantee', '$adding_date');";
...
?>
    <form method="post" action="" id="genpro-add-form">
        ...
        <table>
            <tr valign="top">
                <th><input type="text" placeholder="Serial" name="genpro_serial"/></th>
...
function genpro_products_list_checkout($atts) {
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if(get_option('genpro_option_name')){
            $check_product_query =  "select * from wp_genpro_products where serial = '$genpro_serial' && guarantee = '$genpro_guarantee'";
        }else{
            $check_product_query =  "select * from wp_genpro_products where serial = '$genpro_serial'";
        }    
